...
const ToggleButton = styled.span`
  ...
  &::after {
    ...
    display: block;
    top: -12px;
    left: ${props => (props.on ? 16 : 0) + (props.on && props.clicked ? -6 : 0)}px;
    content: '';
    cursor: pointer;
    background: ${props => props.theme.bg};
    border: 2px solid ${props => props.theme.border};
    width: ${props => props.clicked ? 30 : 24}px;
    height: 24px;
    border-radius: 24px;
  }
`
...
export default class Toggle extends Component {
  ...
  getTheme = () => {
    const { on, disabled, hover } = this.state
    switch(true) {
      case disabled && !on:
  ...
  render() {
    const { on, clicked, disabled } = this.state
    const { label, labelFalse, labelTrue } = this.props
      ...
      <ThemeProvider theme={theme}>
        ...
        >
          ...
          >
            ...
            <ToggleButton on={on} clicked={clicked} />
