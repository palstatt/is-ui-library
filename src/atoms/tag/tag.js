...
const AddTagContainer = styled.div`
  ...
  cursor: pointer;
  transition: .075s ease;
  border: 2px solid ${props => props.theme.border ? props.theme.fg : props.theme.bg};
  transition: .2s ease;
...
export class AddTag extends Component {
  ...
  render() {
    ...
    return (
      <ThemeProvider theme={themes[active && !error ? 'white' : error ? 'warning' : 'black']}>
        <AddTagContainer
          error={error}
          onMouseEnter={this.handleMouseEnter}
          onMouseLeave={this.handleMouseLeave}
        >
          <Appear in={active}  component={<MaterialIcon onClick={this.handleClick}>local_offer</MaterialIcon>} />
          <Grow in={click} component={<TitleInput onChange={this.handleChange} onKeyPress={this.handleKeyPress} value={value} />} />
          <Rotate in={active} component={<AddButton onClick={this.handleClick} active={click}>add</AddButton>} />
        </AddTagContainer>
      </ThemeProvider>
    )
  }
  ...
  render() {
    ...
    return (
      <ThemeProvider theme={themes[theme]}>
        <TagContainer onMouseEnter={this.handleMouseEnter} onMouseLeave={this.handleMouseLeave}>
          <MaterialIcon>local_offer</MaterialIcon>
          <Title>{title}</Title>
          <Appear in={hover} component={<CloseButton onClick={this.props.onClick}>close</CloseButton>} />
        </TagContainer>
      </ThemeProvider>
    )
  }
